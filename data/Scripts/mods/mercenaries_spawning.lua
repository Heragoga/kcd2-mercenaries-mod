    ...
    local ok, err = pcall(function()
        ...
        for i=1, amount do
            ...
            if ent then
                ...
                -- Its own pcall PER STEP: one man's bad gear must not abort the rest of
                -- the batch, and - the reason it is per step rather than one block - the
                -- engine's Lua error carries no line number unless the game was started
                -- with -lua_storedebug 1, so a single pcall reported only
                -- "mercenaries_spawning.lua:0: [Error] Lua error" and named nothing. A
                -- failure here leaves a registered but unfinished merc: alive, in
                -- ActiveMercs, and deaf to every order. Each step now says which it was,
                -- and one failing step no longer skips the five after it.
                local steps = {
                    { "render",    function() mercenaries:EnsureMercIsAlwaysRendered(ent) end },
                    { "clothing",  function() self:EquipMercenary(ent, currentPreset) end },
                    { "weapon",    function() self:EquipMercenaryWeapon(ent, currentWeaponPreset, currentPreset) end },
                    { "interact",  function() self:InjectInteraction(ent) end },
                    ...
                    { "mqwatch",   function() if self.MQWOnHire then self:MQWOnHire(ent) end end },
                    ...
                    { "camp",      function() self:CampOnMercJoined(ent) end },
                }
                for _, s in ipairs(steps) do
                    local ok2, derr = mercenaries.TracedCall(s[2])
                    if not ok2 then
                        System.LogAlways('[Mercenaries] Hire: post-spawn step "' .. s[1] ..
                                         '" FAILED for ' .. entityName .. ': ' .. tostring(derr))
                    end
                end
