...
-- The game's own "immortality" buff (Libs/Tables/rpg/buff.xml: imm=1, persistent). The
-- 5 s health top-up in LogiCampRegen was the only thing standing between him and a raid,
-- and "the quartermaster can be killed" was the report. The camp trader gets it too.
mercenaries.ImmortalBuff = "85aca9c5-ec41-400d-a563-53df7b2399e8"

function mercenaries:MakeImmortal(ent, who)
    if not (ent and ent.soul and ent.soul.AddBuff) then return false end
    local ok = pcall(function() ent.soul:AddBuff(self.ImmortalBuff) end)
    System.LogAlways("[Mercenaries] " .. tostring(who or "camp NPC") .. (ok and " is immortal" or ": immortality buff refused"))
    return ok
end
...
function mercenaries:SpawnQuartermaster(centerPos, facingAngle)
    ...
    local ok, err = pcall(function()
        ...
        -- block it. Both the tent and the house open along raw grid-forward now:
        -- the house's door gable is its local -X end and it is spawned at
        -- facingAngle + pi, and the tent is spawned at whatever yaw puts its
        -- MEASURED doorway on the same bearing (CampPlayerTentYaw). This used to
        -- carry its own copy of the tent's old eyeballed +130 - if the tent is
        -- ever re-aimed, it is CampPlayerTentDoorDeg that moves, not this.
        -- The house still needs more clearance: its body runs ~5m the other way.
        local house = false
        ...
        if ent then
            ...
            self:MakeImmortal(ent, "the quartermaster")
            self._qmImmortalId = ent.id   -- the logistics tick need not do it again
