...
mercenaries.BarksOn = true   -- merc_barks 0/1: the order acknowledgements

function mercenaries:BarksSet(on)
    self.BarksOn = on and true or false
    self._barksLoaded = true
    pcall(function() self:SaveString("MercBarks", self.BarksOn and "1" or "0") end)
    System.LogAlways("[Mercenaries] order barks " .. (self.BarksOn and "on" or "OFF"))
end

function mercenaries:RequestBark(wuid, alias)
    if not wuid or not alias then return end
    if not self._barksLoaded then
        self._barksLoaded = true
        pcall(function() local v = self:LoadString("MercBarks"); if v == "0" then self.BarksOn = false end end)
    end
    if not self.BarksOn then return end
...
function mercenaries:LowPriorityMonitorLoopBody()
    ...
    -- Keep anything built since the last sweep out of the next save. Rides this loop
    -- rather than a timer of its own, because a pending timer is itself saved.
    if mercenaries.NoSaveSweepMaybe then pcall(function() mercenaries:NoSaveSweepMaybe() end) end
    if next(mercenaries.ActiveMercs) then
...
-- Mod Initialization
-- One un-pcall'd throw in OnGameplayStarted silently skips EVERYTHING after it. On
-- 2026-09-19 that was the whole load: the store restored its 61 tags, then something
-- between the outfit load and UpkeepLoad threw, so RebuildMercCacheDelayed and
-- RestoreCampDelayed were never even armed - no merc cache, no roster rebuild, no camp,
-- and the mercs that were hired afterwards were registered but never finished, hence
-- "braindead, no commands work". The engine's Lua error carries no line number without
-- -lua_storedebug 1, so each load step is pcall'd and NAMED instead.
function mercenaries:LoadStep(label, fn)
    local ok, err = mercenaries.TracedCall(fn)
    if not ok then
        System.LogAlways('[Mercenaries] LOAD STEP "' .. tostring(label) .. '" FAILED: ' .. tostring(err))
    end
    return ok
end

-- pcall tells you a step failed; it does not tell you WHERE. The engine's Lua error
-- carries no line number unless the game is started with -lua_storedebug 1, and the file
-- it names is the chunk that armed the call, not the one that threw - which is why
-- "ClearAnyLeftoverCamp FAILED: mercenaries.lua:0" pointed at this wrapper.
--
-- The `debug` library IS present in KCD2 (vanilla's own Scripts/Debug/DebugUtils.lua uses
-- debug.traceback), so xpcall with a traceback handler gives the real call stack with
-- function names. Falls back to plain pcall if debug is ever missing.
function mercenaries.TracedCall(fn, ...)
    if debug and debug.traceback then
        local args = { ... }
        return xpcall(function() return fn(unpack(args)) end,
                      function(e)
                          local tb = ""
...
function mercenaries:OnGameplayStarted(actionName, eventName, argTable)
    ...
    -- Entities the engine RESTORED out of the save predate this session, so nothing has
    -- flagged them. The part that matters is the fatal one: an entity of a class this mod
    -- defines aborts a mod-less LoadGame outright, so it must never reach the next save.
    -- Delayed a beat so the entity system has finished restoring.
    Script.SetTimerForFunction(2000, "mercenaries.NoSaveSweepTick")
    ...
    self:LoadStep("GearLoadState", function() self:GearLoadState() end)
    ...
    self:LoadStep("LoadArcherState", function() self:LoadArcherState() end)
    ...
    self:LoadStep("LoadOrderState", function() self:LoadOrderState() end)
    self:LoadStep("HoldFormUpLoad", function()
        local v = self:LoadString("MercHoldFormUp")
        if v == "shape" then self.HoldFormUp = "shape" elseif v == "1" then self.HoldFormUp = true elseif v == "0" then self.HoldFormUp = false end
    end)
    self:LoadStep("ArcherHoldFireLoad", function()
        local v = self:LoadString("MercArcherHoldFire")
        if v == "0" then self.ArcherHoldFire = false elseif v == "1" then self.ArcherHoldFire = true end
    end)
    ...
    self:LoadStep("UpkeepLoad", function() self:UpkeepLoad() end)
    ...
    self:LoadStep("ClearAnyLeftoverCamp", function() self:ClearAnyLeftoverCamp() end)

    -- Load the quartermaster logistics state (tiredness / food / drink / wages).
    self:LoadStep("LogiLoad", function() self:LogiLoad() end)
    ...
    -- ...and only now that the resync has cleared BanditCampColumn and the shared camp-role
    -- tables: Aleksej's beat camp, from the beat number the loaded save carries. Deliberately not
    -- inside AlxOnLoad - a camp built before that wipe loses its column and its roles. See
    -- AlxRebuildAfterLoad for why the wake token cannot be relied on to do it.
    pcall(function() mercenaries:AlxRebuildAfterLoad() end)

    self:LoadStep("ReleaseSpeakingLock", function() self:ReleaseSpeakingLock() end)
    ...
    if self.SchedEnabled and self.SchedRegisterAll then
        self:LoadStep("SchedRegisterAll", function() self:SchedRegisterAll() end)
        self:LoadStep("SchedStart", function() self:SchedStart() end)
    elseif self.SchedArmLegacy then
        self:LoadStep("SchedArmLegacy", function() self:SchedArmLegacy() end)
    else
        self:ChainArm("MonitorLoop", 1000)
        self:ChainArm("CombatScanLoop", 300)
        self:ChainArm("LowPriorityMonitorLoop", 5000)
...
-- The store entity is the mod's ONLY save footprint; mercenaries_saving.lua is the
-- SaveString/LoadString API on top of it, so it has to be defined first.
...
-- Lays camp decoration on the ground plane under it instead of spawning it level.
-- After camp.lua: it is called from SpawnCampPropModel and reads its footprint helpers.
Script.LoadScript("Scripts/mods/mercenaries_campfit.lua")
-- The vegetation the engine cannot see, baked from the level files into per-tile tables
-- (tools/bake_vegetation.py). After campfit: the fit and nudge helpers call into it.
Script.LoadScript("Scripts/mods/mercenaries_campveg.lua")
...
-- Camera + geometry audit for camp placement (docs/camp-inspection.md). After camp.lua,
-- because it reads the footprint helpers and the placement log that file writes.
Script.LoadScript("Scripts/mods/mercenaries_campshot.lua")
