...
--
-- Food and wages are deliberately worth the SAME number of days per body, which is the only
-- comparison the after-action report makes. A day of food is FeedRatio units per man and a
-- day of wages is one merc-day per man, so parity is simply
-- LootPerKillWages = LootPerKillFood * FeedRatio. Keep them in step if either moves.
mercenaries.LootPerKillFood  = 0.3
mercenaries.LootPerKillDrink = 0.1
mercenaries.LootPerKillWages = 2.4             -- merc-days, converted at WagePerTier.medium
...
function mercenaries:LogiCampRegen(dt)
    ...
    pcall(function()
        ...
        if qm and qm.soul then
            ...
            if self._qmImmortalId ~= qm.id and self.MakeImmortal then
                self._qmImmortalId = qm.id
                self:MakeImmortal(qm, "the quartermaster")
            end
        end
    end)
end
...
function mercenaries:LogiRemoveUpgrade(which)
    ...
    elseif k == "wall" then
        local gates = 0
        pcall(function() gates = self:GateCount() or 0 end)
        -- gates count, so taking the palisade down also clears up after a camp that has
        -- already been left with one standing on its own
        had = (self.WallRuns and #self.WallRuns or 0) > 0 or gates > 0
        -- The gates go WITH the wall. A gate is how a run is finished - it is hung on the
        -- wall's own end and the run is carried to its far edge - so a wall taken down and
        -- a gate left behind is a doorway standing in a field with nothing either side of
        -- it. The gateway is a tile OF the wall and goes the same way. Gates keep their
        -- own entry in UpgRemovable for taking them out of a wall that stays.
...
-- A gate is bought per gate, not once per camp: each purchase places one more, so a
-- camp with three ways in costs three times. Placement is the same aim-and-click the
-- tower and cart use.
function mercenaries:LogiBuyGate()
    if not (self.CampActive and self.CampBuildOrigin) then
        Game.SendInfoText('merc_info_camp_not_active', false, 0, 3); return
    end
    if (self:GateCount() or 0) >= self.GateMax then
        Game.SendInfoText('merc_info_gate_limit', false, 0, 4); return
    end
...
-- Shut them and the raid watch stands down (mercenaries_raids.lua).
function mercenaries:LogiToggleGates()
    if (self:GateCount() or 0) == 0 then
        Game.SendInfoText('merc_info_gate_none', false, 0, 3); return
    end
    local anyOpen = false
    for _, g in ipairs(self.Gates or {}) do
        if g.open then anyOpen = true; break end
    end
