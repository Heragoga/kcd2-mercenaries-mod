...
function mercenaries:MonitorDistanceAndTeleport()
    local ok, err = pcall(function()
        ...
        -- A straggler is not a problem until the player is actually going somewhere, and
        -- while a menu is up nobody can close the gap anyway. See PlayerGoingSomewhere.
        if self.PlayerGoingSomewhere then
            local going = self:PlayerGoingSomewhere()
            self:PlayerStandingNote("the teleporter", not going)
