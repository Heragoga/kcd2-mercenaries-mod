...
-- `width` is the opening: the hole the wall builder cuts, the span the blocking segment
-- seals, and the mesh's own body length. `ox`/`oxOpen` are where that body sits along
-- the panel relative to the mesh's origin, so a gate spawned by its opening's centre
-- lands on the opening rather than beside it. All three are READ OFF THE MESHES by
-- tools/measure_gate.py - width was declared 4.0 for meshes that measure 3.06 and 5.48,
-- which is where the daylight beside every gate came from, and GateWallDist was then
-- dialled to -0.6 to hide it (moving the hole to the gate's other side rather than
-- closing it).
mercenaries.GateStyles = {
    { n = "wooden_d",  open = GATE .. "gate_wooden_d.cgf",
                       closed = GATE .. "gate_wooden_d_closed.cgf",
                       width = 3.06, ox = 0.03, oxOpen = 0.02 },
    { n = "road_bar",  open = BAR .. "barrier_road_a_opened.cgf",
                       closed = BAR .. "barrier_road_natural_closed.cgf",
                       width = 5.48, ox = 0.15, oxOpen = 0.28 },
}
...
-- Clearance between a snapped gate's end and the palisade it butts onto. Now that the
-- width and the two offsets are measured rather than guessed, 0 really is flush, and
-- this is a deliberate 5 cm lap - the wall kit's own pieces are modelled a few
-- centimetres over-length for the same reason, so a joint on sloping ground closes up
-- instead of opening a hairline. The old -0.6 was compensating for a 0.94 m error in
-- the declared gate width and is not a clearance at all.
mercenaries.GateWallDist = -0.05
...
-- How far this mesh's body sits along the panel from its own origin. The two states are
-- separate meshes and need not agree (the road barrier's differ by 0.13 m), so a gate
-- that swung would otherwise shuffle sideways as it opened.
function mercenaries:GateOx(open, g)
    local st = (g and g.style) or gateStyle(self)
    return tonumber(open and (st.oxOpen or st.ox) or st.ox) or 0
end

-- Where the MESH goes for a gate whose opening is centred on `c`, facing mesh-yaw `yaw`
-- (that is, with GateYawFix already applied - the mesh's own +X lies along `yaw`).
-- Everything else - the blocking segment, the colliders, the saved record - stays on the
-- opening's centre; only the prop is nudged onto it.
function mercenaries:GateMeshPos(c, yaw, open, g)
    local ox = self:GateOx(open, g)
    if ox == 0 then return { x = c.x, y = c.y, z = c.z } end
    return { x = c.x - math.cos(yaw) * ox, y = c.y - math.sin(yaw) * ox, z = c.z }
end
...
local function gateSpawnEnt(self, g)
    ...
    local pos = self:GateMeshPos({ x = g.x, y = g.y, z = g.z + (self.GateSink or 0) },
                                 yaw, g.open, g)
    local params = {
...
function mercenaries:GatePlaceSpec()
    -- The ghost has to carry the gate's own two corrections or the preview is not the
    -- gate. rz is GateYawFix: the placement engine draws the ghost at the raw aim angle
    -- while gateSpawnEnt turns the mesh a quarter further, so without it the projection
    -- stood ACROSS the gate you were about to get - edge-on from where you aim, which
    -- reads as no projection at all. y is the mesh offset: the engine's local frame turns
    -- with the aim, and a world shift of -ox along the mesh's own +X is local y = -ox.
    local ox = self:GateOx(false)
    return {
        parts = { { model = self:GateModel(false), x = 0, y = -ox, z = 0,
                    rx = 0, ry = 0, rz = math.rad(self.GateYawFix or 0) } },
        validMaterial = nil,
