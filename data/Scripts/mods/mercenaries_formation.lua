...
function mercenaries:LeaderUnstick(ent)
    ...
    pcall(function()
        ...
        -- Mounted-safe: moves his horse rather than dismounting him. See MercPlaceAt.
        moved = self:MercPlaceAt(ent, tp) and true or false
        if moved then pcall(function() self:NoteTeleport(ent) end) end
    end)
    System.LogAlways("[MercForm] stuck leader " .. tostring(ent and ent.GetName and ent:GetName()) ..
...
function mercenaries:FormationLoopBody()
    if not (next(mercenaries.ActiveMercs) and player) then return end
    -- Per call, not one block. These three run every ~0.22s, and when one threw (2026-09-19,
    -- after a roster restore of 20 men) it took the other two with it on every tick: the
    -- formation never assembled and exactly one man followed. The engine's Lua error has no
    -- line number without -lua_storedebug 1, so the label is the only thing that names it.
    -- Rate-limited: this is a loop, and an every-tick failure must not fill the log.
    local steps = {
        { "UpdateFormationLeader", mercenaries.UpdateFormationLeader },
        { "UpdateFormationFront",  mercenaries.UpdateFormationFront  },
        { "FollowStatsTick",       mercenaries.FollowStatsTick       },
    }
    for _, st in ipairs(steps) do
        local ok, err = mercenaries.TracedCall(st[2], mercenaries)
        if not ok then
            self._formErrAt = self._formErrAt or {}
            local now = 0
            pcall(function() now = System.GetCurrTime() or 0 end)
            if (now - (self._formErrAt[st[1]] or -999)) > 10.0 then
                self._formErrAt[st[1]] = now
                System.LogAlways('[MercForm] FormationLoop step "' .. st[1] ..
                                 '" FAILED: ' .. tostring(err))
            end
        end
    end
