...
-- WALL TIME, and it matters here more than anywhere else in the mod.
-- System.GetCurrTime() is the ENGINE clock, and a crossing is exactly the thing that runs it
-- fast - measured at x29.4 during a time skip. So TravelCoolSecs, a 3-second hold, expired in
-- a TENTH of a real second and the crossing state flipped on and off two to three times a
-- second for the whole journey. Each flip toggled _G.MercIdle, which turned the formation off
-- and on again, which re-elected and re-hauled the leader:
--
--   11:51:38.69 crossing begins / 38.92 the crossing is over / 39.17 begins / 40.06 over ...
--   [MercForm] leader stuck - hauling him ... 401m apart, mounted   (then 487m, then 557m)
--
-- The detection heuristics keep the engine clock - they measure motion, and the position
-- samples they compare are on that same clock. Only the HOLD is wall time.
local function twReal()
    if os and os.clock then
        local t
        if pcall(function() t = os.clock() end) and t then return t end
    end
    local t = 0
...
function mercenaries:TravelWatchTick()
    ...
    if why then
        ...
        self.FastTravelLastReal = twReal()
        if not self._twOn then
            self._twOn = true
            twLog("the player is being carried (" .. why .. ")")
            self:TravelBegin(why)
        end
    elseif self._twOn and self.FastTravelLastReal
           and (twReal() - self.FastTravelLastReal) >= (self.TravelCoolSecs or 3.0) then
        self._twOn = false
        twLog("the crossing is over")
        self:TravelEnd()
    end

    -- Outside the branch above: a sample the detector cannot call either way leaves `why`
    -- nil while the crossing is still running (it ends on TravelCoolSecs, not on one quiet
    -- sample), and the party must not be dropped for those.
    if self._twOn then self:TravelCarry(now) end
end
...
-- A RIDER CANNOT BE MOVED WITH SetPos. The horse is a separate entity, it does not come
-- with him, and the horse lifecycle in follow.xml re-finds MercenaryHorse_<name> BY NAME
-- anywhere in the world and ForceMounts him back onto it - so a man teleported out of the
-- saddle is pulled straight back to wherever his horse was left. Across a crossing that is
-- half the map, which is the "my mercs vanished when I fast travelled on horseback" report.
--
-- MOVE THE HORSE, THEN. The rider is attached to it and comes along, which is how the
-- player's own mount crosses with him. A man on foot is moved himself, and his horse - if
-- the lifecycle has not caught up and despawned it yet - is brought along beside him so the
-- ForceMount-by-name above cannot drag him back.
--
-- NOTHING IS DESTROYED HERE, and that is deliberate. The first version dismounted each man
-- and removed his horse; it crashed the game 17s into the first mounted crossing it ran on
-- (2026-09-19). Nothing else in this mod destroys a horse with a rider on it: follow.xml's
-- lifecycle only removes one from its UNMOUNTED arm and only after an 8-poll (~4s)
-- countdown, and the orphan sweep requires the owner dead or the player on foot. There is
-- even a DespawnHorseByName whose comment reads "deferred despawn". Twenty removals in one
-- frame, one line after ForceDismount on each rider, broke that rule twenty times over.
-- See docs/travel-detection.md.
mercenaries.TravelCarryEvery = 3.0   -- seconds between re-placements during a crossing; 0 = far end only

-- Scattered behind him rather than on him: a dozen men on one point is a physics pile, and
-- the formation sorts them out within a tick.
function mercenaries:TravelScatterSpot(pp)
...
function mercenaries:TravelPlaceAt(ent, pp)
    if not (ent and pp) then return false end
    return self:MercPlaceAt(ent, self:TravelScatterSpot(pp))
end

-- Keep the party with the player FOR THE DURATION, not only at the far end. A crossing can
-- be interrupted part-way by a road encounter, and the falling edge can be missed outright
-- if the detector loses the signal - both strand the company wherever it last stood. Doing
-- it on a slow cadence makes that self-healing.
--
-- Deliberately does NOT call NoteTeleport: that feeds the stall ladder, which escalates at
-- three teleports inside 45s, and one crossing would trip it for every man. The far end
-- stamps it once, which is what that ladder is for.
function mercenaries:TravelCarry(now)
    local every = self.TravelCarryEvery or 0
    if every <= 0 then return end
    if self._twCarryAt and (now - self._twCarryAt) < every then return end
    self._twCarryAt = now
    if not next(self.TravelWith or {}) then return end
    local pp
    pcall(function() pp = player:GetWorldPos() end)
    if not pp then return end
    for id in pairs(self.TravelWith) do
        local e
        pcall(function() e = System.GetEntity(id) end)
        if e then self:TravelPlaceAt(e, pp) end
    end
end
...
function mercenaries:TravelTakesAlong(ent)
    ...
    if self.HoldActive or self.EscortEnt then return false end
...
function mercenaries:TravelBegin(why)
    ...
    -- The first carry happens on this same tick; see TravelWatchTick.
    self._twCarryAt = nil
...
function mercenaries:TravelWatchOnLoad()
    ...
    self._twCarryAt, self.FastTravelLastReal = nil, nil
...
function mercenaries:TravelCarrySet(line)
    local n = tonumber(tostring(line or ""):match("[%d%.]+"))
    self.TravelCarryEvery = n or 0
    twLog(self.TravelCarryEvery > 0
          and string.format("the company is kept with you every %.1fs while you cross", self.TravelCarryEvery)
          or "the company is only brought to you at the far end of a crossing")
end
