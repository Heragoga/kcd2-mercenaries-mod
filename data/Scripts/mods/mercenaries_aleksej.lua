...
function mercenaries:AlxOnLoad()
    ...
    do
        ...
        -- The name half is optional. AlxRebuildAfterLoad needs the beat number on its own, and a
        -- record written before the leader had a name still says which beat was live.
        local n = tonumber(string.match(s, "^(%d+)") or "")
        local name = string.match(s, "^%d+|(.+)$")
        self.AlxLoadBeat = n or 0
...
-- The camp is stood back up HERE, not by the wake token, because the token can be serviced
-- BEFORE this load chain runs. Measured 2026-09-19, the same autosave loaded twice in one
-- session: once with AlxOnLoad 0.8s AHEAD of "camp up", once 3.4s BEHIND it. On the second run
-- AlxOnLoad read the camp the wake had just built as the previous session's, took it down, and
-- nothing re-issues the token - so clearing a camp and then reloading a save from before the
-- fight left the site empty for the rest of the session.
--
-- A camp built ahead of the load chain is half-built anyway: BanditCampColumn and the shared
-- camp-role tables are still the last session's until BanditCampResync wipes them, taking the
-- marching column and the sit/eat/sleep roles with them. So this is called from OnGameplayStarted
-- AFTER that resync, and AlxSpawnBeat's idempotence covers the other order - a token that has
-- not arrived yet finds the camp already standing.
--
-- It reports no progress and takes no authority from the session that saved: AlxLiveBeat is read
-- out of the loaded save, and that record is 0 from the moment that save's leader was noted down.
function mercenaries:AlxRebuildAfterLoad()
    local n = self.AlxLoadBeat or 0
    self.AlxLoadBeat = 0
    if n <= 0 or self.AlxCamp then return end
    aLog("load: beat " .. n .. " was live in this save - standing its camp up now")
    self:AlxSpawnBeat(n)
end
