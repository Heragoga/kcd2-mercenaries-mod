...
function mercenaries.WallBuildTick()
    ...
    pcall(function()
        ...
        -- Near the first corner the run is closing, and the preview shows the join it
        -- would make. That beats a gate snap: a ring has no use for one here.
        local shut = pos and self:WallCloseSnapAt(pos) or nil
        if shut then pos = { x = shut.x, y = shut.y, z = shut.z } end
        local snap = (not shut) and pos and self:WallGateSnapAt(pos) or nil
...
function mercenaries:WallCommitRun()
    ...
    if #m >= 2 then
        ...
        -- same entry and nothing downstream can count them apart. Runs restored from a save
        -- carry no type and are read as timber, which is the common case.
