...
local function place(name, x, y, m, sy, a)
    pcall(function()
        UIAction.SetPos(EL, 0, name, { x = x, y = y, z = 0 })
        if m then
            UIAction.SetScale(EL, 0, name, { x = sc(m), y = sc(sy or m), z = 100 })
        end
        if a then UIAction.SetAlpha(EL, 0, name, a * 100) end
        if not BL.shown[name] then UIAction.SetVisible(EL, 0, name, true) end
    end)
    BL.frame[name] = true
end
...
local function sweep()
    for name in pairs(BL.shown) do
        if not BL.frame[name] then
            pcall(function() UIAction.SetVisible(EL, 0, name, false) end)
        end
    end
...
-- ---------------------------------------------------------------- the corner hints
--
-- The two screens draw one column of nudges - [U] Camp over [H] Command - and while both
-- are closed that column is the only thing the mod puts on screen. It lived in the bottom
-- right, which is where KCD2 shows its own pickup and objective messages, so players were
-- losing game text behind it. Where it sits and whether it draws at all are now settings:
--
--   merc_hints on|off              both nudges
--   merc_hints_pos <where>         topright (default) | compass | topleft
--                                  | bottomright | bottomleft
--   merc_hints_pos <x> <y>         anywhere, in stage units on a 1280x720 stage
--
-- They live on `mercenaries` rather than in either driver because the camp screen borrows
-- BL.hintEnabled while it is up - CUShow turns it off so only one nudge speaks at a time -
-- and a player's setting must not be something a screen can quietly overwrite.
...
-- The setting is shared, so whichever screen is currently drawing has to be told.
...
-- Per-screen switches, under the shared one: someone who wants only the camp nudge can
-- turn this one off on its own.
...
function mercenaries:BLHintUpdate()
    -- Not gated on having men: the nudge is how you find the interface at all, and it kept
    -- vanishing whenever the company was empty - between hiring, after losses, on a fresh
    -- save. The camp screen's has never been gated and this one matches it.
    self:HintLoad()
    BL.hint = BL.hintEnabled and (mercenaries.HintsOn ~= false)
    if BL.on then return end
    if BL.hint then
        if self:BLElement(true) then
            Script.SetTimerForFunction(600, "mercenaries.BLDeferred")
        else
            self:BLLayout()
        end
    else
        self:BLElement(false)
    end
end
...
function mercenaries:BLLayout()
    ...
    if not BL.on then
        if BL.hint then hintRow("hint_lbl") end
        sweep()
        applySlowmo()
        applyQamBlock()
        return
...
giveKey = function(k)
    ...
    pcall(function()
        if back then
            ...
            System.ExecuteCommand("unbind " .. k)
        end
    end)
end
