...
function mercenaries:AmbushMonitor()
    ...
    -- Not while the map is carrying him: a trigger box crossed at fast-travel speed sprang
    -- the scene under a player who was not there to see it, with none of the game's own
    -- "you were ambushed" interruption - he arrived at his destination with the gang already
    -- on him. The box stays armed; he can walk into it later.
    if self._twOn then return end
    local pp = player and player:GetWorldPos()
...
mercenaries:DevCommand("merc_ambush_new",     "mercenaries:AmbushNewScene('%line')", "Start a new ambush scene: merc_ambush_new [name]")
mercenaries:DevCommand("merc_ambush_archer",  "mercenaries:AmbushPlace('archer')", "Mark archer spots: left-click places a barrel, right-click discards them all, merc_ambush_save keeps them")
mercenaries:DevCommand("merc_ambush_melee",   "mercenaries:AmbushPlace('melee')",  "Mark melee spots: left-click places a barrel, right-click discards them all, merc_ambush_save keeps them")
mercenaries:DevCommand("merc_ambush_trigger", "mercenaries:AmbushPlace('trigger')","Mark trigger areas: four barrels per area (place 8 for two areas), then merc_ambush_save")
mercenaries:DevCommand("merc_ambush_save",    "mercenaries:AmbushSave()",          "Save the spots placed since the last placement command into the scene")
mercenaries:DevCommand("merc_ambush_dump",    "mercenaries:AmbushDump()",          "Print the scene as pasteable Lua")
mercenaries:DevCommand("merc_ambush_test",    "mercenaries:AmbushTest('%line')",   "Spawn the scene for real: merc_ambush_test [group] (default looter) - archers on archer spots, melee on melee spots")
mercenaries:DevCommand("merc_ambush_test_clear", "mercenaries:AmbushClearTest()",  "Remove the enemies spawned by merc_ambush_test")
mercenaries:DevCommand("merc_ambush_clear",   "mercenaries:AmbushClear()",         "Remove all ambush markers and test enemies")
mercenaries:DevCommand("merc_ambush_status",  "mercenaries:AmbushStatus()",        "List the live ambush scenes and whether triggers are armed")
mercenaries:DevCommand("merc_ambush_where",   "mercenaries:AmbushWhere()",         "Why isn't it firing? Prints your position against every trigger box")
mercenaries:DevCommand("merc_ambush_arm",     "mercenaries:AmbushSetEnabled('%line')", "Arm or disarm live ambush triggers: merc_ambush_arm 0 | 1")
mercenaries:DevCommand("merc_ambush_despawn", "mercenaries:AmbushDespawnActive()", "Remove the ambush currently on the ground")
