...
--
-- WHO SHOOTS WHOM
--   Defender archers run "mod_enemies": they shoot mod-spawned enemies - the besiegers - and
--   nothing else, so they never touch the player or their mercs.
...
function mercenaries:RaborschPlaceRow(row, origin)
    ...
    if row.kind == "tower" then
        ...
        -- The garrison's own. mod_enemies: they shoot besiegers and nobody else.
        local before = #(self.TowerStations or {})
        self:SpawnTowerStation(pos, yaw, { mode = "mod_enemies" })
    ...
    elseif row.kind == "cart" then
        ...
        self:SpawnArcherCart(pos, yaw, { mode = "mod_enemies" })
...
function mercenaries:RaborschSpawnFoot(origin)
    local S = self.RBQ
    local total = self:RaborschFootCount()
    ...
    for i = 1, total do
        local grp = self.RaborschFootGroups[math.random(#self.RaborschFootGroups)]
    ...
    rLog(string.format("%d foot: %d in camp, %d on patrol (squad of %d x %.1f)",
        total, campers, walkers, self:RaborschSquadSize(), self.RaborschFootRatio))
end
...
function mercenaries:SpawnRaborsch()
    ...
    for _, row in ipairs(layout) do
        pcall(function() self:RaborschPlaceRow(row, origin) end)
    end
