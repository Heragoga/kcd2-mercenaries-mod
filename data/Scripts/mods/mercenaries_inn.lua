...
function mercenaries:InnPropsSpawn()
    ...
    for i, p in ipairs(self.InnProps) do
        ...
        -- Terrain-only, or a barrel beside a table snaps onto the table. See CampSeatOnTerrain.
        if self.CampSnapToGround then pos = self:CampSnapToGround(pos) end
        if self.CampSeatOnTerrain then pos = self:CampSeatOnTerrain(pos) end
        local e
        pcall(function()
...
function mercenaries:SpawnInnStation()
    ...
    if self.CampSeatOnTerrain then origin = self:CampSeatOnTerrain(origin) end
...
function mercenaries:SpawnCampInn(center)
    ...
    -- Whichever of the three routes above produced it, the spot has never been checked
    -- against buildings or against what the ground is made of. The cached tile can be stale,
    -- ForgeFindFlattest can be overruled, and the last fallback is a blind 10 m offset - which
    -- is how a judged survey found the whole tavern pitched in a streambed with an ale barrel
    -- hanging clear of the water.
    if spot and self.CampNudgeClearOfStructures then
        spot = self:CampNudgeClearOfStructures(spot, 2.5)
    end
    -- ...and make sure it is still part of the camp. CampStationSpot can hand back a CACHED
    -- tile from an older layout that no distance rule ever saw, which is how the mess tables
    -- ended up 20-25 m away on the far side of a stream.
    if spot and self.CampLeashToCamp then
        spot = self:CampLeashToCamp(spot)
    end
    ...
    -- Everything on the ground is tested where it will stand (see CampSatelliteSpot); the
    -- claims are made after the whole tavern is down, so its own tables do not refuse its
    -- own stools. Tabletop dressing rides on its table and is not tested.
    local claimsBefore = self.CampPlacedFoot and #self.CampPlacedFoot or 0
    local placed = {}
    local dropped = {}   -- { fwd, lat } of every ground prop left out
    for _, L in ipairs(self.InnStationLayout) do
        local w = wp(L)
        local onGround = (L.up or 0) < 0.3
        if onGround and self.CampSatelliteSpot then
            w = self:CampSatelliteSpot(w, ang + math.rad(L.rz or 0), L.m, spot, "tavern " .. tostring(L.n), claimsBefore)
            if not w then table.insert(dropped, { L.fwd or 0, L.lat or 0 }) end
        elseif not onGround then
            -- Dressing rides on a table or a barrel; without it, it would hang in the air.
            -- A table is 1.6 m long, so anything within a metre along it is its own.
            for _, d in ipairs(dropped) do
                if math.abs((L.fwd or 0) - d[1]) < 1.0 and math.abs((L.lat or 0) - d[2]) < 0.6 then w = nil end
            end
        end
        if not w then
            -- left out, and so is whatever would have sat on it
        elseif tostring(L.n):match("^stool") then
            ...
            if wuid then
                table.insert(placed, { pos = w, yaw = ang, model = L.m, what = "tavern " .. tostring(L.n) })
            ...
            if e then
                ...
                if onGround then
                    table.insert(placed, { pos = w, yaw = ang + math.rad(L.rz or 0), model = L.m, what = "tavern " .. tostring(L.n) })
                end
            end
        end
    end
    -- Now the tavern announces the ground it took, so the tent rings placed after it (a
    -- rebuild puts stations down first) route round its tables and barrels.
    if self.CampClaimFoot and self.CampPropFootHalf then
        for _, p in ipairs(placed) do
            self:CampClaimFoot(p.pos, p.yaw, self:CampPropFootHalf(p.model, 0), p.what)
        end
    end
