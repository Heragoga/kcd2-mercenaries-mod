...
-- Mesh detail by crowd size, measured in game rather than guessed: at 100 only ~5m stays high
-- quality, 200 is medium, 300 leaves ~3m detailed and the rest clay figures. Bigger crowd ->
-- HIGHER ratio (detail drops sooner) -> affordable. Below 20 it makes no odds, so the engine
-- is left to decide.
--
...
function mercenaries:LodRatioAutoSet(v)
    ...
    if self.LodRatioAuto then self:LodRatioAutoApply() end
end
