...
mercenaries.CmdHelpSections = {
    ...
    { "ORDERS",     { "merc_hold", "merc_hold_end", "merc_hold_formup", "merc_escort", "merc_escort_end",
    ...
    { "CAMP",       { "merc_camp_make", "merc_camp_break", "merc_camp_recall",
                      ...
                      "merc_camp_remove", "merc_camp_party", "merc_gate_remove", "merc_wall_undo_run",
                      "merc_camp_compass",
    ...
    { "OPTIONS",    { "merc_difficulty", "merc_upkeep", "merc_encounters", "merc_patrols", "merc_patrols_perday",
                      "merc_status_icons", "merc_horses", "merc_horses_max", "merc_autodismount", "merc_nobump", "merc_barks",
                      "merc_archer_holdfire", "merc_camp_key", "merc_cmd_key", "merc_lod_quality",
                      "merc_mqstash", "merc_travel_stow", "merc_travel_carry", "merc_travelprobe", "merc_travelstate", "merc_horsestats", "merc_whystand", "merc_formprobe", "merc_roster", "merc_roster_nosave",
                      "merc_hide_others" } },
...
-- A key handed back with nothing to restore is bound here: the console has no unbind,
-- and a key left on merc_camp_ui after merc_camp_key j still opened the screen.
cmd("merc_noop",    "local _ = 0",                    "Does nothing (a released key lands here)")
...
cmd("merc_hold_formup", "mercenaries:HoldFormUpSet('%line')", "How a hold order draws the men up: 0 = stand fast where they are (default), 1 = a block, shape = the company's own formation (line, wedge, circle...)")
...
cmd("merc_chaseflee",    "mercenaries:ChaseFleeingSet(mercenaries:CmdBool('%line'))", "Run down enemies who break and flee instead of letting them go: 0 | 1 (default 1)")
...
cmd("merc_barks",        "mercenaries:BarksSet(mercenaries:CmdBool('%line'))",           "The men's spoken acknowledgements of an order: 0 | 1 (saved, default 1)")
cmd("merc_archer_holdfire", "mercenaries:ArcherHoldFireSet(mercenaries:CmdBool('%line'))", "Archers refuse a target with a friend within arm's reach of it: 0 | 1 (saved, default 1)")
cmd("merc_camp_key",     "mercenaries:CampUiKeySet('%line')",  "Rebind the camp screen (default u): merc_camp_key <key>")
cmd("merc_cmd_key",      "mercenaries:BLKeySet('%line')",      "Rebind the command screen (default h): merc_cmd_key <key>")
cmd("merc_gate_remove",  "mercenaries:GateRemoveNearest()",    "Take down the gate nearest you")
cmd("merc_wall_undo_run", "mercenaries:WallUndoRun()",         "Take down the last stretch of wall")
...
cmd("merc_travel_carry", "mercenaries:TravelCarrySet('%line')", "Seconds between keeping the company with you mid-crossing; 0 = far end only")
...
-- NOT ours, however much the name looks like it: "SpawnedAnimal_<Species>_<hash>" is the
-- VANILLA AnimalSpawner's naming. A never-modded save (playline0/autosave233) carries 110
-- of them. It was briefly added to the list above on 2026-09-14 after a decoded save
-- showed boar and roe deer alongside the mod's leftovers - which would have had the mod
-- deleting the game's own wildlife and flagging it out of the player's saves.
...
-- What the mod would leave in a save. Since the store landed that is at most ONE
-- entity, so the interesting number is the second one: how many tags it holds.
-- Legacy per-tag BasicEntity savers are still counted, because a save made by an
-- older build has them until MigrateLegacy has run over it once.
function mercenaries:CountSaverEntities()
    ...
    pcall(function()
        ...
        for _ in pairs(System.GetEntitiesByClass("mercenaries_Store") or {}) do n = n + 1 end
        for _, e in pairs(System.GetEntitiesByClass("BasicEntity") or {}) do
            if e and e.GetName and e:GetName() == "MercStore" then n = n + 1 end
        end
    end)
    pcall(function()
        self:SaverMap()
        for _ in pairs(self.SaverValues or {}) do tags = tags + 1 end
    end)
    return n, tags
end
...
function mercenaries:PurgeSaversNow(quiet)
    ...
    local gone = 0
    -- The store entity itself, tags and all. This is the one thing standing between a
    -- save and a zero footprint, so it goes rather than just being emptied.
    pcall(function()
        -- The store is a vanilla BasicEntity called MercStore now; older saves may still
        -- carry the mercenaries_Store one, which is the entity that broke mod-less loads.
        for _, e in pairs(System.GetEntitiesByClass("mercenaries_Store") or {}) do
            pcall(function() System.RemoveEntity(e.id) end)
            gone = gone + 1
        end
        for _, e in pairs(System.GetEntitiesByClass("BasicEntity") or {}) do
            if e and e.GetName and e:GetName() == "MercStore" then
                pcall(function() System.RemoveEntity(e.id) end)
                gone = gone + 1
            end
        end
    end)
    -- Legacy per-tag savers from a save made before the store landed.
    pcall(function()
        local pfx, plen = self.SaverPrefix, string.len(self.SaverPrefix)
        for _, e in pairs(System.GetEntitiesByClass("BasicEntity") or {}) do
            local name = e and e:GetName()
