...
function mercenaries:SpawnArcherCart(atPos, atYaw, opts)
    ...
    st.archerSpecs = {}
    -- ArcherCartCrew thins the crew where a full three would be too much - see the marsh island,
    -- where four carts of three archers is a killing field for a small company. nil = all three.
    local crew = self.ArcherCartCrew or #self.ArcherCartArcherLayout
    for slot, a in ipairs(self.ArcherCartArcherLayout) do
        if slot > crew then break end
...
-- Remove every cart THE PLAYER OWNS. ArcherCarts is one shared list, and breaking camp - which
-- fast travel does on its own - used to strip a bandit camp of its carts mid-contract from the
-- other side of the map. A cart with st.group belongs to an encounter (the same test the budget
-- above uses) and only that encounter's own teardown may remove it.
function mercenaries:ClearArcherCarts()
    local kept = {}
    for i = #self.ArcherCarts, 1, -1 do
        local st = self.ArcherCarts[i]
        if st and st.group then
            table.insert(kept, 1, st)
        else
            self:ArcherCartClearOne(st)
        end
    end
    self.ArcherCarts = kept
    System.LogAlways("[ArcherCart] player carts removed (" .. #kept .. " encounter cart(s) left standing)")
end
