...
-- Millimetres. A wall joint has 3 cm of slack in it (WallJointSlack), so rounding every
-- corner to the nearest centimetre spent a third of that budget before the rebuild had
-- laid a single piece.
local function fmt(n) return string.format("%.3f", n or 0) end
...
-- A wall CORNER is more than a point, and saving only the point is why a reloaded
-- palisade came back a little way from where it stood with the gap at its gate opened up.
-- Three fields the rebuild reads were being dropped on the floor:
--
--   aim     where the player actually pointed. WallRefitRun deliberately fits to the aim
--           and not to the mark, because the fit can only ever SHORTEN an edge: fit a
--           mark to itself and every rebuild shortens it again from where the last one
--           left it. With the aim gone that is precisely what each load did, so the run
--           crept inward a little every time it was reloaded.
--   spread  set on the two marks that bring a ring's join home. The edge into a mark
--           carrying it is laid in "close" mode; without it the join is tiled as an
--           ordinary edge and the ring shifts.
--   flush   per run, not per point (below): the run ends on a GATE's edge, which is
--           deliberately not a whole number of tiles from the corner before it. The
--           refit leaves that last corner alone only while it knows, and the last edge
--           is filled to reach it only while it knows. Losing it both moved the corner
--           and stopped the fill - the widening gate gap, twice over.
--
-- Written as three more comma fields, so unpackPoints (still used for the castle
-- gateways) reads the first three and ignores the rest, and a save from before this
-- loads with the extras simply absent.
local function packMarks(list)
    local out = {}
    for _, p in ipairs(list or {}) do
        local a = p.aim or p
        table.insert(out, table.concat({ fmt(p.x), fmt(p.y), fmt(p.z),
...
local function unpackMarks(s)
    local out = {}
    if not s or s == "" then return out end
    for chunk in string.gmatch(s, "[^;]+") do
        local f = {}
        for v in string.gmatch(chunk, "[^,]+") do f[#f + 1] = v end
        if f[3] then
            local p = { x = tonumber(f[1]), y = tonumber(f[2]), z = tonumber(f[3]) }
            if f[6] then
                p.aim = { x = tonumber(f[4]), y = tonumber(f[5]), z = tonumber(f[6]) }
            end
            if f[7] == "1" then p.spread = true end
            table.insert(out, p)
        end
    end
    return out
...
-- ==== save ====
-- Writing a list that has just become EMPTY is the whole point of a save taken after a
-- teardown, and SaveString refuses an empty string outright - so every one of those
-- writes failed silently and the save kept the layout that had just been pulled down.
-- Take the last gate out of a camp and it was gone until you reloaded, and then it was
-- back. DefForget already used a space for exactly this reason; every list write does now.
local EMPTY = " "

local function putList(self, tag, s)
    self:SaveString(tag, (s ~= nil and s ~= "") and s or EMPTY)
end
...
function mercenaries:DefSave()
    ...
    -- Runs are packed one per "|" field, each still a ";"-separated corner list, so a
    -- single-run string written by an older save still unpacks as run one. The closed
    -- flags ride alongside as a matching list, and the per-run options (flush end, wall
    -- type) as another - kept out of QMWallClosed so an old save still parses.
    local runs, closed, opts = {}, {}, {}
    for _, r in ipairs(self:WallAllRuns()) do
        table.insert(runs, packMarks(r.pts))
        table.insert(closed, r.closed and "1" or "0")
        table.insert(opts, (r.flushEnd and "1" or "0") .. "," ..
                           tostring(r.wtype or self.WallTypeIdx or 3))
    end
    putList(self, "QMWallPts",    table.concat(runs, "|"))
    putList(self, "QMWallClosed", table.concat(closed, "|"))
    putList(self, "QMWallOpts",   table.concat(opts, "|"))
    ...
    putList(self, "QMCastleGw", packPoints(self.CastleGateways or {}))
    ...
    putList(self, "QMGates", packPoses(gates))
    ...
    putList(self, "QMTowers", packPoses(towers))
    ...
    putList(self, "QMCarts", packPoses(carts))
end
...
-- The SAVED wall runs, in the same { pts, closed } shape WallAllRuns returns. Read by
-- IsSpotNearWall while the live runs are down: an upgrade purchase tears the camp
-- (and with it the palisade) down before laying the new one out, so the tiles are
-- chosen in a window where WallRuns is empty and the wall is still plainly there.
-- Bearings are not recovered here - this is only ever asked for distances.
function mercenaries:DefSavedWallRuns()
    if not self:DefBelongToCurrentCamp() then return nil end
    local out = {}
    pcall(function()
        ...
        for i, raw in ipairs(runsRaw) do
            ...
            if #pts >= 2 then
                table.insert(out, { pts = pts, closed = (closedRaw[i] == "1") })
            end
        end
    end)
    return out
...
function mercenaries:DefRestoreBody()
    ...
    local runsRaw, closedRaw, optsRaw = {}, {}, {}
    for chunk in string.gmatch((self:LoadString("QMWallPts") or "") .. "|", "([^|]*)|") do
        table.insert(runsRaw, chunk)
    end
    for chunk in string.gmatch((self:LoadString("QMWallClosed") or "") .. "|", "([^|]*)|") do
    ...
    for chunk in string.gmatch((self:LoadString("QMWallOpts") or "") .. "|", "([^|]*)|") do
        table.insert(optsRaw, chunk)
    end
    ...
    for i, raw in ipairs(runsRaw) do
        ...
        if #pts >= 2 then
            ...
            local flush, wtype = nil, nil
            local f, w = string.match(optsRaw[i] or "", "^([^,]*),([^,]*)$")
            if f == "1" then flush = true end
            wtype = tonumber(w)
            table.insert(self.WallRuns, { pts = pts, closed = (closedRaw[i] == "1"),
                                          flushEnd = flush, wtype = wtype })
    ...
    if #self.WallRuns > 0 then
        ...
        if ty and self.WallTypes[ty] then
            self.WallTypeIdx = ty
            self.WallSegLen = nil
            -- every per-type placement figure WallSetType loads, not just two of them.
            -- ox and yaw were missing: a restored run kept whatever the last type
            -- selected had left behind, so a stone curtain came back laid along the
            -- wrong axis and a palisade came back shifted down its own run.
            self.WallUp  = self.WallTypes[ty].up  or 0
            self.WallLat = self.WallTypes[ty].lat or 0
            self.WallOx  = self.WallTypes[ty].ox  or 0
            self.WallYawFix = self.WallTypes[ty].yaw or 0
        end
