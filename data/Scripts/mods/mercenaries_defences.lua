...
local function packPoses(list)
    local out = {}
    for _, p in ipairs(list or {}) do
        table.insert(out, fmt(p.x) .. "," .. fmt(p.y) .. "," .. fmt(p.z) .. "," .. fmt(p.yaw))
    end
    return table.concat(out, ";")
end
...
local function unpackPoses(s)
    ...
    for chunk in string.gmatch(s, "[^;]+") do
        ...
        if x then table.insert(out, { x = tonumber(x), y = tonumber(y), z = tonumber(z), yaw = tonumber(a) }) end
    end
    return out
end
...
function mercenaries:DefSave()
    ...
    self:SaveString("QMWallPts",    packPoints(self.WallMarks))
...
function mercenaries:DefRestore()
    ...
    local pts = unpackPoints(self:LoadString("QMWallPts"))
    if #pts >= 2 then
        self.WallMarks  = pts
        self.WallClosed = (self:LoadString("QMWallClosed") == "1")
    ...
    System.LogAlways(string.format("[Defences] restored: %d wall corner(s), %d tower(s), %d cart(s)",
        #pts, #unpackPoses(self:LoadString("QMTowers")), #unpackPoses(self:LoadString("QMCarts"))))
end
