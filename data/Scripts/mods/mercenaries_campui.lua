...
local function place(name, x, y, m, sy, a)
    ...
    if st.m ~= m or st.sy ~= sy then
        -- z is NOT optional: without it the scriptbind throws, and it threw on every
        -- scaled clip of every frame (112 caught errors in one open, kcd.log 2026-09-19).
        -- Nothing looked broken because this place() calls SetVisible first, so the clips
        -- still showed - at whatever scale they last had.
        pcall(function()
...
function mercenaries:CUStowSave()
    ...
    table.sort(out)
    -- A space when storage is EMPTY, never "": SaveString refuses an empty string, so
    -- taking the last thing back out of storage never cleared the tag and a reload handed
    -- it straight back - a free second gate, for ever. Same sentinel DefForget uses, and
    -- CUStowLoad reads a space as nothing stowed. See docs/gates.md.
    local packed = table.concat(out, ",")
    pcall(function() self:SaveString("MercCampStowed", packed ~= "" and packed or " ") end)
end
...
function mercenaries:CULayout()
    ...
    -- down, so the corner would put it half on top of the logistics panel. The chosen
    -- corner governs the command interface's prompt, which has a clear corner to sit in.
    ...
    if not CU.on then
        -- Closed: nothing. There is no idle prompt any more - see the header in
        -- mercenaries_blui.lua for why.
        sweep()
        return
    end
    -- Open: the wheel's own Return spoke carries U, so the prompt only speaks when the
    -- wheel is shut.
    if not CU.open and CU.hintEnabled and (mercenaries.HintsOn ~= false) then
        hintRow("hint_lbl", true)
    end
...
-- Closed draws nothing, but the element stays loaded: see BLHintUpdate for why taking it
-- down would put a 600ms delay on every press of U.
-- Sweeps, never loads - see BLHintUpdate.
...
function mercenaries:CUAct(slot, key)
    ...
    elseif key == "remove" then
        -- Anything that comes down WITH this one, counted before it goes. Taking the
        -- palisade down takes its gates with it (a gate is hung on a wall's end, so one
        -- left behind is a doorway standing in a field) - and they were paid for too, so
        -- they are stowed alongside it rather than lost.
        local also = {}
        if slot.key == "wall" then also.gate = self:CUCount("gate") or 0 end
        ...
        for k, n in pairs(also) do
            if n > 0 and RESTORE[k] then
                self.CampStowed[k] = math.max(self.CampStowed[k] or 0, n)
                stowed = true
            end
        end
        if stowed then
...
giveKey = function(k)
    ...
    pcall(function()
        ...
        else System.ExecuteCommand("bind " .. k .. " merc_noop") end   -- the console has no unbind
    end)
end
...
function mercenaries:CUShow()
    ...
    -- ...and take the command movie off the layer, for the reason in BLShow.
    if mercenaries.BLElement then
        pcall(function() self:BLElement(false) end)
    end
...
function mercenaries:CUHide()
    ...
    -- Nothing is reopened here, and the command screen is not touched: closing a screen
    -- closes it. This used to call BLHintUpdate to bring the command screen's idle corner
    -- prompt back, and once that prompt was removed the call did nothing but load the
    -- command movie onto the layer the camp one is still holding.
end
...
function mercenaries:CUOnLoad()
    ...
    self.CUPlaceWatchArmed = false
    self:CUStowLoad()
    -- A key the player chose (merc_camp_key) beats the shipped one: U is also the weather
    -- overhaul's key and the toggle-hood mod's, and a camp that cannot be pitched is the
    -- loudest kind of conflict.
    local ov = self.UiKeyLoad and self:UiKeyLoad("MercCampKey")
    if ov then atlas().hideKey = { ov, string.upper(ov) } end
...
-- A saved key override, or nil. One token of letters and digits; "-" or "" mean none.
function mercenaries:UiKeyLoad(tag)
    local v
    pcall(function() v = self:LoadString(tag) end)
    if type(v) ~= "string" then return nil end
    v = string.lower(v)
    if not string.match(v, "^[%w_]+$") then return nil end
    -- Nothing longer than a key name ("np_enter" is 8) and nothing of ours is a key.
    if string.find(v, "^merc_") or string.len(v) > 12 then return nil end
    return v
end

-- merc_camp_key <key>: rebind the camp screen. Takes effect now and is kept with the save.
function mercenaries:CampUiKeySet(line)
    ...
    if not k or k == "%line" then
        log("the camp screen is on '" .. tostring(A.hideKey[1]) .. "' - merc_camp_key <key> to change it")
        return
    end
    k = string.lower(k)
    if not string.match(k, "^[%w_]+$") then log("'" .. k .. "' is not a key name"); return end
    if CU.held[A.hideKey[1]] then giveKey(A.hideKey[1]) end
    pcall(function() self:SaveString("MercCampKey", k) end)
    A.hideKey = { k, string.upper(k) }
    takeKey(k, "merc_camp_ui")
    pcall(function() self:CUHintUpdate() end)
    log("camp screen key is now '" .. k .. "'")
end
