...
-- Each test in its own guard. These are EXCLUSION predicates on the 0.22s formation
-- loop: one of them throwing used to abort UpdateFormationLeader on every tick, so the
-- formation never assembled and exactly one man followed the player (2026-09-19). A
-- predicate that cannot answer must not be able to do that - it is treated as "does not
-- exclude" and named once every 10s, which is strictly better than losing the squad.
local function guard(self, label, fn)
    local ok, res = pcall(fn)
    if ok then return res end
    self._eligErrAt = self._eligErrAt or {}
    local now = 0
    pcall(function() now = System.GetCurrTime() or 0 end)
    if (now - (self._eligErrAt[label] or -999)) > 10.0 then
        self._eligErrAt[label] = now
        System.LogAlways('[MercForm] IsFormationEligible: "' .. label ..
                         '" threw, treating as not-excluded: ' .. tostring(res))
    end
    return false
end
...
function mercenaries:IsFormationEligible(ent, wuid)
    ...
    -- A handle the roster rebuild replaced is the likeliest thrower here: ActiveMercs can
    -- still hold an entity the engine has since removed, and any method on it errors.
    if ent and guard(self, "IsAliveAndWell", function() return not self:IsAliveAndWell(ent, false) end) then return false end
    if guard(self, "IsMercInCampProper", function() return self:IsMercInCampProper(wuid) end) then return false end
    if guard(self, "CampActorGet", function() return self:CampActorGet(wuid) end) then return false end
    ...
    if self.FollowFormationSuppressed
       and guard(self, "FollowFormationSuppressed", function() return self:FollowFormationSuppressed(wuid) end) then
        return false
    end
...
function mercenaries:FollowGhostSweep()
    ...
    if self.PlayerGoingSomewhere then
        local going = self:PlayerGoingSomewhere()
        self:PlayerStandingNote("the ghost-latch sweep", not going)
        if not going then return end
    end
    local now = fhNow()

    -- Collected, not cured inline. A ghost latch is a RACE, and a race does not happen to
    -- thirty men in the same instant - if the whole company's follow trees have gone silent
    -- together that is the world (a menu, a crossing, a hitch), and evicting and re-firing
    -- every one of them cannot make a frozen tree run. It only guarantees that none of them
    -- ever stamps a slot again, which is what keeps the storm alive.
    local ghosts = {}
                ...
                if not busy then
                    ...
                    if not last or (now - last) >= (self.FollowGhostLogEvery or 10.0) then
                        ...
                        System.LogAlways(string.format(
                            ...
                            .. "(scheduler %.1fs ago, %.0fm from the player)",
    ...
    if #ghosts >= systemic then
        -- Bounded, not nothing - the same call the follow watch's systemic branch makes, and
        -- for the same reason: if the trees really have all died, standing down every pass
        -- leaves the company standing for ever. Six re-fires per 3s sweep instead of thirty
        -- is the difference between a repair and the storm.
        local n = math.min(#ghosts, self.FollowWatchSystemicRepair or 6)
        if not self._fgSysAt or (now - self._fgSysAt) >= (self.FollowWatchSystemicLog or 15.0) then
            self._fgSysAt = now
            System.LogAlways("[MercForm] " .. #ghosts .. " of " .. squad ..
                             " follow tree(s) silent at once - that is the world, not a race; re-firing " ..
                             n .. " per pass")
        end
        local keep = {}
        for i = 1, n do keep[i] = ghosts[i] end
        ghosts = keep
    else
        self._fgSysAt = nil
    end
    for _, e in ipairs(ghosts) do pcall(function() self:FollowStalled(e) end) end
end
...
function mercenaries:DismountVerify()
    ...
    -- Nor while he is not going anywhere: the drift test has nothing to measure and the
    -- far-but-stationary test indicts the whole rear of a column. See PlayerGoingSomewhere.
    if self.PlayerGoingSomewhere then
        local going = self:PlayerGoingSomewhere()
        self:PlayerStandingNote("the follow watch", not going)
        if not going then self._dvPos = nil; return end
    end
