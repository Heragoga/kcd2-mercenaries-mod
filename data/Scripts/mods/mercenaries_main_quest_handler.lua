...
function mercenaries:MonitorMainQuestLoop()
    ...
    -- Wall time when travelwatch has stamped it: System.GetCurrTime() is the engine clock
    -- and a crossing runs it ~30x fast, so this 3s debounce expired almost at once and
    -- _G.MercIdle flickered for the whole journey. See mercenaries_travelwatch.lua.
    local ftAgo = nil
    if self.FastTravelLastReal and os and os.clock then
        pcall(function() ftAgo = os.clock() - self.FastTravelLastReal end)
    end
    if ftAgo == nil and self.FastTravelLastDetected then
        ftAgo = currentRealTime - self.FastTravelLastDetected
    end
    if ftAgo and ftAgo < 3.0 then
