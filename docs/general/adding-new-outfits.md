...
> `PackageMod.bat` finds the game on its own, in any Steam library. If it cannot (a non-Steam copy, an unusual folder), run `set KCD2_DIR=D:\path\to\KingdomComeDeliverance2` in the same command prompt first, or copy `tools/local.paths.txt.example` to `tools/local.paths.txt` and fill in the `game=` line.
