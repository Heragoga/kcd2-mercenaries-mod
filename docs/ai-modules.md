...
| `combat_archer_dynamic` | Shoot at the right distance, where "right" comes from `mercenaries.ArcherBands` (per ranged weapon — see docs/archers.md): stand-and-fire inside the band; **beyond `keepMax`, walk in to just outside `keepMin` and then fire**; inside `keepMin`, **step back then fire a burst** (step-fire kiting — the fire always follows, so a blocked retreat can't leave the archer idle); inside `meleeRange` or out of ammo, sidearm burst until the gap reopens past `reengageRange`. | merc archers (skirmish stance), enemy archers |
...
- `UpdateRangedCombatData(data, wuid)` — target-alive, distance, out-of-ammo (all ammo classes), stance validity (friend archers only), leash (friend: `MeleePlayerLeash()`, the same scaled player leash the melee module uses; enemy: own target 60 m), the band edges (`meleeRange` / `reengageRange`), and the retreat **and approach** points for the dynamic module.
  The friend leash used to be a flat 40 m, which reproduced the melee module's old bug exactly — below the acquisition range, so a rear-rank archer was handed a target and told to break off on the same tick, forever. `archer_scheduler.xml` now reads `deTargetDist` from `MercLeashes` for the same reason.
  Approach points are refused if they would land more than 35 m from the player (a friendly archer must not walk himself out of his own leash) or across a camp wall, and are cached on the same terms as retreat points.
...
- **Never hold a pose with `UnstanceAction`; use `UnstanceElement`.** `UnstanceAction` starts the animation and returns, so the pose belongs to nobody: when the tree is torn down (combat replacing `camp_actor`, a role change, break-camp) the fragment keeps playing and the NPC walks, follows and fights **still in it** - the "merc floats away sitting" report. `UnstanceElement` takes the same attributes plus `enabled` and wraps the hold as its child, so leaving the arm - for any reason - plays the `Out` fragment. Same for `StanceElement`: put the hold inside it, never after it.
- **Give an animated arm a chance to unwind before failing the tree.** `camp_actor` raises `inCampAnim` while it is inside a Stance/Unstance element; the `campYield` gate then waits (up to 3 s, ending the instant the flag clears) instead of failing on the spot, and the arm's hold is a poll `Loop` on `$campYield` rather than one long `Wait`. The merc stands up and *then* leaves. A man with no pose still yields on the same tick as before.
