...
* [Putting your own marker on the world map](map-marker.md) (The map is a Scaleform movie, not a game system: nine values into `UIAction.SetArray("ApseMap", -1, "PoiMarkers", ...)` and a POI is on the map. The mod no longer draws any; kept as the record, along with the compass marker it still ships)
...
* [Detecting that the player is indoors](interior-detection.md) (The engine's own `interior` area label, authored into the building prefabs and read by the stealth buffs: the full label vocabulary, the BT / Skald / Lua ways to read it, what it does and does not cover, why `System.IsPointIndoors` is inert in KCD2, and the squad park it drives)
...
* [Your own men stop shoving and trampling you](collision-ghosting.md) (`merc_nobump`: why there is no collision-damage setting anywhere in KCD2 - the three parameters that look like one are dead Crysis leftovers - and the physics collision-class filter that is the real lever, with the full `g_PhysicsCollisionClass` bit table read out of the DLL)
...
### Camp
* [What counts as "the ground"](ground-guard.md) (Why props landed on roofs and patrolling mercs on buildings: every placement ended in a raycast that took the FIRST surface it met. How a ray hit tells terrain from a brush from an entity, the two chokepoints that carry ~100 call sites, and the two failsafes for the case where the guard is wrong about a region)
...
* [The camp trader](camp-trader.md) (The Trader upgrade: a market stall and a sutler you buy from and sell loot to, the baked item-price table behind it, and why it is not the game's own shop screen)
* [Camp amenities](camp-amenities.md) (The player's own chest beside the bed - sharing the real stash through the vanilla `masterStash` link, not a second inventory - plus a working drying rack and smokehouse rebuilt from the butcher's prefabs entirely in Lua)
