...
**Recall**: `merc_camp_recall` brings the whole squad to your current position immediately, from anywhere, and resumes following — this does **not** break camp; the tents and fire stay standing, just empty, until you explicitly break camp. Players who want a hotkey can bind one themselves with `bind <key> merc_camp_recall`. The mod itself binds **no F-key** (it used to auto-bind F4 every load; that was removed together with the bench/torture F-key binds after players kept firing things by accident — no keybinds in release, by decree).
