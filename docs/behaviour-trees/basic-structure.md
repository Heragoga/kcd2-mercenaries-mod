...
The most common reason to call another behaviour tree is to trigger a combat interrupt. First, register the target tree as a `SmartBehaviorTemplate` in your `SmartEntity__so_interrupt__yourmodid.xml` as described in [make-npc-brain.md](../xml/make-npc-brain.md). Then use this sequence to fire it:
